...
class MainContents extends Component {
  ...
  handleOnClickCelcius = () => {
    ...
    if (temp_c !== this.state.temperature) { // check if the user is clicking on the same button to prevent rerendering
      this.setState({
        temperature: `${this.props.current.avgtemp.temp_c} °C`,
        changed: true
      });
  ...
  handleOnClickFarenheit = () => {
    ...
    if (temp_f !== this.state.temperature) { // check if the user is clicking on the same button to prevent rerendering
      this.setState({
